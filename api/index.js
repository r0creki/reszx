...
function generateKey() {
  ...
  const part = () => Array.from({ length: 5 }, () =>import { supabase } from "../lib/supabase.js";
import { verifyUser, signUser } from "../lib/auth.js";
import jwt from "jsonwebtoken";
import axios from "axios";
import qs from "querystring";
...
export default async function handler(req, res) {
  ...
  if (action === "workink-callback") {
    try {
      ...
      if (!session || session.used) return res.redirect("/?error=invalid_session");
      ...
      if (!pendingToken) return res.redirect("/?error=no_token");
      ...
      await supabase
        .from("users")
        .update({ total_keys: supabase.raw('total_keys + 1') })
        .eq("discord_id", user.id);
  ...
  if (action === "redeem") {
    ...
    try {
      ...
      if (!keyData) return res.status(404).json({ error: "Invalid key" });
